...
#
# 类似于py-mini-orm 的Query 
# 只不过是用property 和setattr(self, key, value)
#
# 而openerp 则是用__getattr__, __getitem__
#
class browse_record(object):
    def __init__(self, cr, uid, id, table, cache, context=None, list_class = None, fields_process={}):
        ...
        self._uid = uid
        self._id = id
        self._table = table # table 就是一个表的类实例
        self._table_name = self._table._name
        self._context = context
        self._fields_process = fields_process

        cache.setdefault(table._name, {}) # 初始化cache
        self._data = cache[table._name] #初始化_data
...
#            raise BrowseRecordError(_('Object %s does not exists') % (self,))
    ...
    # 如果外部没有使用到序列的需求的话, 也就是object['key'] 这种形式
    # __getitem__ 完全可以放到__getattr__里面去!!
    def __getitem__(self, name):
        # 'id' 在150行就必定初始化了,直接返回
        if name == 'id':
            return self._id

        # _data 中不存在 name 这个key 所对应的对象, 构建之
        if name not in self._data[self._id]:
            # build the list of fields we will fetch

            # fetch the definition of the field which was asked for
            if name in self._table._columns: # 本类字段
                col = self._table._columns[name]
            elif name in self._table._inherit_fields: # 父类字段
            ...
            for data in datas:
                ...
                for n, f in ffields:
                    if f._type in ('many2one', 'one2one'):
                        ...
                        new_data[n] = data[n]
                self._data[data['id']].update(new_data) # 更新属于data['id']的内容
        return self._data[self._id][name]

    def __getattr__(self, name):
#       raise an AttributeError exception.
        return self[name] # 会调用 __getitem__(self, name)
    ...
    def __str__(self): # print object 时的内容
...
class orm_template(object):
    ...
    # 所有字段首先放在 ir_model_data, 
    # - name: 一般是view 中'id'=__name__
    # - module: 自身模块的名字, 如hr_expense ,表示由这个模块产生
    # - model: name实际存放的表名
    # - res_id: name 在model 中的id
    ...
    def browse(self, cr, uid, select, context=None, list_class=None, fields_process={}):
        ...
        # 07-01 传入cache， 为空，怎么能获取对象链接池呢？ 就是object.field.field1 之类
        # 07-05 cache 为空表名每次都重新构造一次browse_record 这个对象, 有点浪费!!!!
        #
...
# TODO: set order
    ...
    def _update_store(self, cr, f, k):
        ...
        cr.execute('select id from '+self._table) # 获取table 全部id
        ids_lst = map(lambda x: x[0], cr.fetchall())
        while ids_lst:
            iids = ids_lst[:40] # 取前四十条记录
            ids_lst = ids_lst[40:]
    ...
    def _auto_init(self, cr, context={}):
        ...
        create = False
        todo_end = []
        self._field_create(cr, context=context) # 创建字段
        if not hasattr(self, "_auto") or self._auto: # hasattr(class, _field) 是否有某字段
