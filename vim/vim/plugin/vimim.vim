...
" -----------------------------------
function! s:vimim_four_char(keyboard)
" -----------------------------------
    let fuzzy_double_pinyin = 0
    let sheng_mu = "[aeiouv]" |" zaze => 2_char (zhanzheng:war)
    if len(a:keyboard) == 4 
    \&& strpart(a:keyboard,1,1) =~ sheng_mu
    \&& strpart(a:keyboard,3,1) =~ sheng_mu
...
" ---------------------------------------
    ...
    if s:vimim_four_char(a:keyboard) > 0
        let fuzzy = '.*'
    endif
    let start = '^'
...
" --------------------------------------------
    ...
    call filter(results, 'v:val !~ "#$"')
    return s:vimim_filter(results, a:keyboard)
endfunction
...
" -----------------------------------------
    ...
    let length = len(a:keyboard)
    if s:chinese_input_mode < 2
        if s:extreme_fuzzy_flag < 1
        \&& length > 0
        \&& length < 5          |" 4_char => 4_chinese
            if s:vimim_four_char(a:keyboard) > 0
                let length = 2  |" 4_char => 2_chinese
            endif
            let patterns = '\s\+.\{'. length .'}$'
...
"<!-- BEGIN STANDARD TAG - 300 x 250 - Sneakme.net: Sneakme.net - DO NOT MODIFY -->
"<IFRAME FRAMEBORDER=0 MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=NO WIDTH=300 HEIGHT=250 
"SRC="http://ad.xtendmedia.com/st?ad_type=iframe&ad_size=300x250&section=620120"></IFRAME>
"<!-- END TAG --><!-- BEGIN STANDARD TAG - 300 x 250 - Sneakme.net: Sneakme.net - DO NOT MODIFY -->
"<IFRAME FRAMEBORDER=0 MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=NO WIDTH=300 HEIGHT=250 
"SRC="http://ad.xtendmedia.com/st?ad_type=iframe&ad_size=300x250&section=620120"></IFRAME>
"<!-- END TAG -->
